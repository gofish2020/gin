...
func TestListOfRoutes(t *testing.T) {
	...
	router.GET("/", handlerTest1) // 这里的路径相当于 /

	// 这里相当于公共的前缀 /users
	group := router.Group("/users")
	{
		group.GET("/", handlerTest2)    // 这里的路径相当于 /users/
		group.GET("/:id", handlerTest1) // 这里的路径相当于 /users/:id ,并且对应的响应函数位handlerTest1
		group.POST("/:id", handlerTest2)
	}
	// "/static" 表示访问的路由
	// "." 访问的url对应的静态文件在，服务器的存储位置（起始位置）+ 拼接上 url中的路径，就是服务器的文件路径
	router.Static("/static", ".")

	// 对所有的方法树进行遍历，汇总出所有的路径
	list := router.Routes()

	for _, l := range list {
		t.Log(l.Path, l.Method)
	}
	assert.Len(t, list, 7)
	assertRoutePresent(t, list, RouteInfo{
		Method:  "GET",
		Path:    "/favicon.ico",
		Handler: "^(.*/vendor/)?github.com/gofish2020/gin.handlerTest1$",
	})
	assertRoutePresent(t, list, RouteInfo{
		Method:  "GET",
		Path:    "/",
	...
	assertRoutePresent(t, list, RouteInfo{
		Method:  "GET",
		Path:    "/users/",
		Handler: "^(.*/vendor/)?github.com/gofish2020/gin.handlerTest2$",
	})
	assertRoutePresent(t, list, RouteInfo{
		Method:  "GET",
		Path:    "/users/:id",
