...
func New() *Engine { // *Engine的本质，其实是对 RouterGroup的额外包装，真正调用执行的方法，其实都是RouterGroup的
...
func Default() *Engine {
	...
	engine := New()                  // 创建引擎对象
	engine.Use(Logger(), Recovery()) // 含义：就是设定两个 HandlerFunc 函数对象，保存到RouterGroup的Handles成员变量上
	return engine
}
...
func (engine *Engine) Handler() http.Handler {
	...
	// 利用http2对 engine又包装了一层
	h2s := &http2.Server{}
	return h2c.NewHandler(engine, h2s) // 最后实际执行的，就是engine对象的ServeHTTP方法
}

func (engine *Engine) allocateContext(maxParams uint16) *Context {
	v := make(Params, 0, maxParams) // key/value 切片数组
...
func (engine *Engine) Use(middleware ...HandlerFunc) IRoutes {
	// 将 middleware 保存到 RouterGroup 中
...
func (engine *Engine) RunUnix(file string) (err error) {
	...
	if engine.isUnsafeTrustedProxies() {
		debugPrint("[WARNING] You trusted all proxies, this is NOT safe. We recommend you to set a value.\n" +
			"Please check https://github.com/gofish2020/gin/blob/master/docs/doc.md#dont-trust-all-proxies for details.")
	}
